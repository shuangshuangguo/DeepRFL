...
from dataset import RFLDataSet
from res_model import RFL_model
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(
        TSNDataSet(args.train_list, num_segments=args.num_segments, new_length=data_length, \
    ...
    val_loader = torch.utils.data.DataLoader(
        TSNDataSet(args.val_list, num_segments=args.num_segments, new_length=data_length, \
                   modality=args.modality,
                   image_tmpl="img_{:05d}.jpg" if args.modality=="RGB" else args.flow_prefix+"{}_{:05d}.jpg",
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, cls_criterion, samp_criterion, sim_criterion, l1_criterion, htri_criterion, optim, epoch)
...
def train(train_loader, model, cls_criterion, samp_criterion, sim_criterion, l1_criterion, htri_criterion, optim, epoch):
    ...
    for m, (full_input, target, _, part_input, part_target, ratio) in enumerate(train_loader):
        assert np.array_equal(target.numpy(), part_target.numpy()), "part video and complete video should have same label"
        data_time.update(time.time() - end)
        batch_size = part_input.size(0)
        part_target = part_target.cuda(async=True)
        ...
        loss = loss_cls + loss_sim + loss_full
        ...
        cls_losses.update(loss_cls.data[0], full_input.size(0))
        prec1, _ = accuracy(part_out.data, part_target, topk=(1,5))
        top1.update(prec1[0], part_input.size(0))
        ...
        if m % args.print_freq == 0:
            print('Epoch: [{0}][{1}/{2}], lr: {lr:.5f}  '
                  ...
                  'prec@1 {top1.val:.3f} ({top1.avg:.3f})'.format(
                   ...
                   data_time=data_time, loss=cls_losses, top1=top1, lr=optim.param_groups[-1]['lr']))
    print('train result: ', top1.avg)
#
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    ...
    if is_best:
        best_name = '_'.join((args.snapshot_pref, 'model_best_', str(state['epoch']), str(state['best_prec1'])+'.pth.tar'))
